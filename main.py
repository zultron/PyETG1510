...
from pyetg1510 import EtherCATMasterConnection, MasterODSpecification, ETG1510Profile, SysLog, LoggingLevel, \
    DiagnosisData, ConfiguredAddressList
...
async def get_etg1510_data(etg1510: ETG1510Profile, index: int = 0xA000):
    ...
    if isinstance(sdo, DiagnosisData):
    ...
    elif isinstance(sdo, ConfiguredAddressList):
        pprint({"Address": sdo.address_list})
...
if __name__ == "__main__":
    ...
    SysLog.rotation_log_configuration(LoggingLevel.DISABLE)
    ...
    asyncio.run(get_etg1510_data(etg1510, 0xA003))
